...
Then(/^the user details are displayed successfully$/) do
  ...
  expect(page).to have_content "I Live at the following afdsfsfdaddress #$houseNo #$street #$city #$postcode"
  ...
  expect('carsasfd').to have_content "#$car".downcase
  ...
  if $isCycling then
    expect($cyclings).to have_content $cycling
  end
